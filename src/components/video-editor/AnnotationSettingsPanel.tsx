...
interface AnnotationSettingsPanelProps {
	...
	onCalloutDataChange?: (calloutData: CalloutData) => void;
	onDelete: () => void;
}
			...
			<div className="mb-6">
				...
				>
					<TabsList className="mb-4 bg-white/5 border border-white/5 p-1 w-full grid grid-cols-4 h-auto rounded-xl">
						<TabsTrigger
							value="text"
							className="data-[state=active]:bg-[#005DE8] data-[state=active]:text-white text-slate-400 py-2 rounded-lg transition-all gap-1 text-[10px]"
						>
					...
					{/* Callout Content */}
					<TabsContent value="callout" className="mt-0 space-y-4">
						<div>
							...
							<textarea
								...
								rows={3}
								className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-slate-200 text-sm placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-[#005DE8] focus:border-transparent resize-none"
							/>
						</div>
						...
						<div className="space-y-4">
							<div>
								...
								<Select
									value={annotation.calloutData?.tailPosition || "bottom"}
									onValueChange={(value) => {
										onCalloutDataChange?.({
											...annotation.calloutData!,
											tailPosition: value as CalloutTailPosition,
										});
									}}
								>
							...
							<div>
								<label className="text-xs font-medium text-slate-200 mb-2 block">
									Tail Offset ({annotation.calloutData?.tailOffset || 50}%)
								</label>
								<Slider
									value={[annotation.calloutData?.tailOffset || 50]}
									onValueChange={([value]) => {
										onCalloutDataChange?.({
											...annotation.calloutData!,
											tailOffset: value,
								...
								<div>
									...
									<Popover>
										<PopoverTrigger asChild>
											...
											>
												<div
													...
													style={{ backgroundColor: annotation.calloutData?.backgroundColor }}
												/>
										...
										<PopoverContent className="w-[260px] p-3 bg-[#1a1a1c] border border-white/10 rounded-xl shadow-xl">
											<Block
												color={annotation.calloutData?.backgroundColor}
												colors={colorPalette}
												onChange={(color) => {
													onCalloutDataChange?.({
														...annotation.calloutData!,
									...
									<Popover>
										<PopoverTrigger asChild>
											...
											>
												<div
													className="w-4 h-4 rounded-full border border-white/20"
													style={{ backgroundColor: annotation.calloutData?.color }}
												/>
												<ChevronDown className="h-3 w-3 opacity-50 ml-auto" />
											</Button>
										</PopoverTrigger>
										<PopoverContent className="w-[260px] p-3 bg-[#1a1a1c] border border-white/10 rounded-xl shadow-xl">
											<Block
												color={annotation.calloutData?.color}
												colors={colorPalette}
												onChange={(color) => {
													onCalloutDataChange?.({
														...annotation.calloutData!,
							...
							<div className="flex items-center justify-between gap-2">
								...
								>
									<ToggleGroupItem
										...
										className="h-8 w-8 data-[state=on]:bg-[#005DE8] data-[state=on]:text-white text-slate-400 hover:bg-white/5 hover:text-slate-200"
									>
					...
					<TabsContent value="image" className="mt-0 space-y-4">
						...
						<Button
							...
							variant="outline"
							className="w-full gap-2 bg-white/5 text-slate-200 border-white/10 hover:bg-[#005DE8] hover:text-white hover:border-[#005DE8] transition-all py-8"
						>
					...
					<TabsContent value="figure" className="mt-0 space-y-4">
						...
						<div>
							...
							<Popover>
								<PopoverTrigger asChild>
									...
									>
										<div
											...
											style={{ backgroundColor: annotation.figureData?.color || "#005DE8" }}
										/>
										<span className="text-xs text-slate-300 truncate flex-1 text-left">
											{annotation.figureData?.color || "#005DE8"}
										</span>
										<ChevronDown className="h-3 w-3 opacity-50" />
									</Button>
								</PopoverTrigger>
								<PopoverContent className="w-[260px] p-3 bg-[#1a1a1c] border border-white/10 rounded-xl shadow-xl">
									<Block
										color={annotation.figureData?.color || "#005DE8"}
										colors={colorPalette}
										onChange={(color) => {
											const newFigureData: FigureData = {
												...annotation.figureData!,
